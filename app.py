...
class BusinessIntelligenceAgent:
  def_init_(self.deals,work_orders):
    self.deals=deals
...
def work_order_summary(self):
  ...
  rerutn "Work order data not available"
...
def win_rate(self):
  if 'stage' in self.deals.columns:
      total=len(self.deals)   won=self.deals[self.deals['stage'].str.lower().str.contains("won")].shape[0] return round((won/total)*100,2) if total>0 else o
  return "Not available"
def query(self,user_input):
  ...
  if "revenue" in q:
    ...
    return "Sorry, I coundn't understand the query."
def main():
  print("Initializing BI Agent...\n")
  deal=work_orders=load_data()
